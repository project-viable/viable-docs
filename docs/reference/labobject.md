## LabObjects ([see examples](../examples/examples-labobjects))
...
All `LabObject`s are part of the `LabObject` group. They take care of that on their own. You should add them to other groups depending on what they are capable of, so that other objects know how to interact with them. See [here](/docs/reference/groups) for what those are and how they should be used..
